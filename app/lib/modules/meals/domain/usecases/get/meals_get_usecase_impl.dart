
import 'package:app/modules/meals/data/exceptions/meals_exceptions.dart';
import 'package:app/modules/meals/data/repository/meals_repository.dart';
import 'package:core/base/data/api/exceptions/network/network_exceptions.dart';
import 'package:core/base/domain/entities/exception_model.dart';
import 'package:dartz/dartz.dart';
import 'package:injectable/injectable.dart';

import '../../models/meal/detail/meal_detail_model.dart';
import 'meals_get_usecase.dart';
import 'meals_get_usecase_params.dart';

@Injectable(as: MealsGetUseCase)
class MealsGetUseCaseImpl implements MealsGetUseCase {
  MealsGetUseCaseImpl(this.mealsRepository);

  final MealsRepository mealsRepository;

  @override
  Future<Either<ExceptionModel, MealDetailModel>> execute(MealsGetUseCaseParams params) async {
    try {
      final meal = await mealsRepository.getMeal(params.id);
      if (meal == null) {
        return Left(ExceptionModel(
          textKey: MealsExceptions.getErrorKey(
            const MealsExceptions.notFoundError()
          ))
        );
      }
      return Right(meal);
    } on Exception {
      return Left(ExceptionModel(
        textKey: MealsExceptions.getErrorKey(
          const MealsExceptions.unexpectedError()
        ))
      );
    } on NetworkExceptions catch (e) {
      return Left(ExceptionModel(
        textKey: NetworkExceptions.getErrorKey(e)
      ));
    }
  }
}

